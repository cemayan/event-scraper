...
func (m MiddlewareSvc) ValidUserId(c *fiber.Ctx) error {
	...
	statusCode, err := m.cb.Execute(func() (interface{}, error) {
		resp, err := m.httpClient.R().
			EnableTrace().
			SetAuthToken(token).
			Post("http://" + m.configs.AUTH_SERVER + "/api/v1/auth/validateToken")
...
func (m MiddlewareSvc) Protected() fiber.Handler {
	...
	return jwtware.New(jwtware.Config{
		SigningKey:   []byte(m.configs.SECRET),
...
func NewMiddleware(httpClient *resty.Client, cb *gobreaker.CircuitBreaker, log *log.Logger, configs *api.AppConfig) Middleware {
	return &MiddlewareSvc{httpClient: httpClient, cb: cb, log: log, configs: configs}
}
